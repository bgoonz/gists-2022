...
export function constructImagePathPrefix(collection, area, skill) {
  const paths = ["assets", collection.name, area.name, skill.name]
...
export function getSkillSearchQuery(collection, area, skill) {
  return [collection.name, area.name, skill.name]
    .filter((name) => name != null)
    .join(" ");
}
...
export function isDialogVisible() {
  return (
    document.documentElement.hasAttribute("data-dialog-count") &&
