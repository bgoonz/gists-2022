...
function loadAssets() {
  window.style = "";
  style += "  h3{ padding: 15px 0 0 0; margin:0; font-size: 15px;} ";
  style +=
    "  h2 a { font-size: 20px; color: #000; margin: 0; text-decoration: none; float: left; }";
  style += "  header { height: 30px; }";
  style += "  header small{ float: right; clear: left; color: #717171;}";
  style += "  header span{ float: right; }";
  style += "  table{ border-spacing:0; border-collapse:collapse }";
  style += "  table tr td{ border:1px solid }";
  style +=
    "  table tr td{ border-collapse:collapse; padding-left: 7px; font-size: 13px; }";
  style += "  table tr td:nth-of-type(1){ width:370px; }";
  style += "  table tr td:nth-of-type(2){ width:50px; }";
  style += "  table tr td:nth-of-type(3){ min-width:260px }";
  style += "  tr.no-pkg-price{ background-color: #fffbe6; }";
  style += "  td.no-opt-features { border: none; }";
  style += "  table tr.price-tbd td:nth-of-type(2){ background: #ffe0e0; }";
  style += "  tr.standard-feat td:first-of-type { background: #ffe0e0; }";
  window.t = "";
  t += "{{#each sections}}";
  t += "  <h3>{{title}}</h3>";
  t += "  <table>";
  t += "    {{#each features}}";
  t += '      <tr class="{{classes}}">';
  t += "        <td>{{{title}}}</td><td>{{price}}</td>";
  t += "          <td>{{#each package}} {{this}}<br> {{/each}}</td>";
  t += "      </tr>";
  t += "    {{else}}";
  t += '    <tr><td class="no-opt-features">No optional features</td></tr>';
  t += "    {{/each}}";
  t += "  </table>";
  t += "{{/each}}";
}
function init() {
  window.mpPrices = {};
  window.mpPrices.features = null;
  window.mpPrices.packages = null;
  window.mpPrices.accessories = null;
  var e = getModelUrls();
  $.when(
    $.getJSON(e["features"]).done(features_callback),
    $.getJSON(e["packages"]).done(package_callback)
  ).then(start);
  var t = "<header>";
  t +=
    '<h2><a href="' +
    ...
    "</a></h2>";
  t += "<span>" + getMSRP() + "</span>";
  t += "<br><small>" + getDateTime() + "</small>";
  t += "</header>";
  t +=
    '<p class="loading" style="padding: 100px 0;text-align: center;">Loading..</p>';
  createPopup(
    t,
    "width=690,height=900",
    getModelName() + " " + getDateString(),
    window.style
  );
}
function features_callback(e) {
  var t = e.model_section_features,
    n = [];
  for (var r in t) {
    var i = t[r]["touts"],
      s = t[r]["title"],
      o = processFeatures(i);
    n.push({ title: s, features: o });
  }
  window.mpPrices.features = n;
}
function package_callback(e) {
  var t = e.model_section_packages,
    n = [];
  for (var r in t) {
    var i = t[r],
      s = i["title"],
    ...
    n.push({
      ...
      package: [i["status"] ? "Standard" : ""],
      status: o,
      classes: u.join(" "),
    });
  }
  window.mpPrices.packages = [{ title: "Packages", features: n }];
  window.mpPrices.accessories = [
    { title: "Accessories", features: processFeatures(e.accessories["touts"]) },
  ];
}
function start() {
  var e = window.mpPrices,
    t = [].concat(e.features, e.accessories, e.packages);
  feats = Handlebars.compile(hereDoc(tmpl));
  phtml = feats({ sections: t });
  $(window.mywindow.document).find("p.loading").remove();
  $(window.mywindow.document).find("body").append(phtml);
}
...
function getDateString() {
  var e = new Date(),
    ...
    n = e.getDate(),
    r = e.getUTCFullYear();
  return r + "-" + (t < 10 ? "0" + t : t) + "-" + (n < 10 ? "0" + n : n);
}
function getMSRP() {
