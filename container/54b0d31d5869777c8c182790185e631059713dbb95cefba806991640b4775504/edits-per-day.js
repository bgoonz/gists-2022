...
const MIN_DISTANCE_SECONDS = 60 * 5; // 5min
...
async function main() {
  try {
    const r = await query(
      `
    select r.created, d.id as document_id, d.locale, r.creator_id
    ...
    for (let { created, creator_id, locale, document_id } of r) {
      ...
      if (!edits[locale][document_id]) {
        edits[locale][document_id] = {};
      }
      if (!edits[locale][document_id][creator_id]) {
        edits[locale][document_id][creator_id] = [];
      }
      edits[locale][document_id][creator_id].push(created);
...
function add(dateStr, edits) {
  ...
  for (const [locale, docEdits] of Object.entries(edits)) {
    ...
    let countEdits = 0;
    let countUsers = 0;
    for (const revisionEdits of Object.values(docEdits)) {
      for (const createds of Object.values(revisionEdits)) {
